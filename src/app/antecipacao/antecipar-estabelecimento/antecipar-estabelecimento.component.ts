...
export class AnteciparEstabelecimentoComponent implements OnInit {
  ...
  resultados = new Datatable<any>({
    table: [
      ...
      { property: 'autorizacao', hideSort: true, description: 'Autorização' },
      {
        property: 'valorVenda',
        hideSort: true,
        description: 'Valor da venda (R$)',
        pipe: 'noSymbolCurrency',
      },
      {
        property: 'valorParcela',
        hideSort: true,
        description: 'Valor da parcela (R$)',
        pipe: 'noSymbolCurrency',
      },
      {
      ...
      },
      {
        ...
        hideSort: true,
        description: 'Valor líquido à antecipar (R$)',
        pipe: 'noSymbolCurrency',
      },
    ],
    data: [],
    ...
    this.antecipacaoService.pesquisarRecebiveisParaAntecipacao(parameters).subscribe(arr => {
      arr.forEach((item: any) => {
        ...
        item.taxaAntecipacao =
          item.taxaAntecipacao !== '-' ? item.taxaAntecipacao + '%' : item.taxaAntecipacao;
      });
  ...
  valorDisponivelNegativo(): number {
    const table = this.resultados.internal.data;
    const val = table.reduce((acc, item) => {
      return acc + (item.selecionado ? (item.valorPagar < 0 ? item.valorPagar : 0) : 0);
    }, 0);
  ...
  solicitar(): void {
    ...
    const selecionados = this.resultados.internal.data.filter(
      row => row.selecionado || row.valorPagar < 0,
    );
