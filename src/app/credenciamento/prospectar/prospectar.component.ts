...
import { Status, StatusDescricao, IIndicacao, IParametrosPesquisa } from '../../interfaces/participante/indicacao';
...
export class CredenciamentoProspectarComponent implements OnInit {
    ...
    search() {
        ...
        this.service.pesquisarIndicacoes(parameters)
            .subscribe(data => {
                this.data = data;
            });
    }
    ...
    credenciar(data: IIndicacao) {
        ...
        const validate = dados && dados.length > 0
            ? this.prompt.confirm(
                `Existe um${edicao ? 'a edição' : ' cadastro'} de credenciamento em andamento e os dados serão perdidos.
    ...
    buttonVisible(data: IIndicacao) {
        return data.canalEntrada !== tipoCanalEntrada.indicaoPorFornecedor
            || data.status !== this.indicacaoStatus.pendente || data.credenciamentoStatus;
    }
}
