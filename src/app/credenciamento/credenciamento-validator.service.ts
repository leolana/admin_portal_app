...
export class CredenciamentoValidatorService {
  ...
  constructor(private service: CredenciamentoService) {}
  ...
  async validaDadosCadastrais() {
    const hasError = [await this.validaFaturamento(), await this.validaTicket()];

    return hasError.filter(error => error).length > 0;
  }

  async validaDadosInstalacao() {
    const hasError = [this.validaHorarioInstalacao()];
  ...
  validaFaturamento() {
    return new Promise(r => {
      this.service.getFaturamentos().subscribe(faturamentosDisponiveis => {
        const faturamentoSelecionado = JSON.parse(sessionStorage.getItem('financeiro'))
          .faturamentoAnual;
  ...
  validaBancos() {
    return new Promise(r => {
      this.service.getBancos().subscribe(bancosDisponiveis => {
