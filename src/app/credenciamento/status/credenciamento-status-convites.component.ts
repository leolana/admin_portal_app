...
export class CredenciamentoStatusConvitesComponent implements OnInit {
    ...
    defineDadosTabela = async () => {
        ...
        this.usuariosService.checaExisteParticipante(emails)
            .subscribe((convitesEMembros: any) => {
                ...
                this.dados.forEach(novoConvite => {
                    ...
                    memberships.forEach((participante: any) => {
                        const usuarioExistente = participante.filter((membro: any) => membro.email === novoConvite.email);
                        ...
                        if (usuarioExistente && usuarioExistente.length) {
                            ...
                            const membroExistente = usuarioExistente.find((membro: any) => membro.memberId === this.idEstabelecimento);
                            if (membroExistente) {
                ...
                this.dados = this.dados.filter(dado =>
                    !usuariosJaVinculados.find(user => user.email === dado.email)
                );
