...
import { CredenciamentoSteps, Horarios, Wizard, CredenciamentoNumeroSteps } from '../../interfaces/credenciamento';
...
export class CredenciamentoDadosInstalacaoComponent implements OnInit, AfterViewInit {
        ...
        if (this.loaded && dadosInstalacao) {
            ...
            if (this.tipoPessoa === TiposPessoa.juridica) {
                ...
                if (socio) {
                    ...
                    edicao || analise ? this.setValueFormContato(dadosInstalacao) : this.setValueFormContato(socio);
    ...
    validForm() {
        Object.keys(this.formEndereco.controls).forEach(k => this.formEndereco.controls[k].markAsDirty());
        Object.keys(this.formContato.controls).forEach(k => this.formContato.controls[k].markAsDirty());

        if (!this.formInstalacao.valid) {
            const errosControlesEndereco = this._obterErrosFormulario(this.formInstalacao.controls.endereco);
