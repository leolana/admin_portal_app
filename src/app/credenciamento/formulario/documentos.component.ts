...
export class CredenciamentoDocumentosComponent implements OnInit {
  ...
  chaveDomicilio(d) {
    return `${d.banco.id}#${d.agencia}#${d.conta}-${d.digito}`;
  }
  ...
  criaControlsParaArquivos(): void {
    ...
    this.domicilioBancario = Object.values(agrupado).map((domicilio: any, i) => {
      ...
      return {
        ...
        descricao: `${domicilio.banco.text} - Ag. ${domicilio.agencia} / Conta ${domicilio.conta}-${
          domicilio.digito
        }`,
        control: control,
      };
  ...
  getDocumentosExistentes() {
    ...
    if (this.arquivosEdicao) {
      this.arquivosEdicao
        .filter(d => !d.id.startsWith('extratosBancarios') && d.id !== 'analise')
        .forEach(documento => {
  ...
  bindFile(property, event, index = null) {
    ...
    if (property === 'extratosBancarios') {
      ...
      domicilios
        .filter(d => this.chaveDomicilio(d) === this.domicilioBancario[index].id)
        .forEach(d => {
          d.newFile = true;
          d.arquivo = null;
        });
  ...
  validForm() {
    Object.keys(this.formDocumentos.controls).forEach(k =>
      this.formDocumentos.controls[k].markAsDirty(),
    );
