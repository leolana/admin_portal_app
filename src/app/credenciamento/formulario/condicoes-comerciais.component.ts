...
import { ICondicaoComercial, CredenciamentoSteps, IOpcoesParcelamento } from '../../interfaces/credenciamento';
...
export class CredenciamentoCondicoesComerciaisComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (Boolean(sessionStorage.getItem('credenciamentoEdicao')) || Boolean(sessionStorage.getItem('credenciamentoEmAnalise'))) {
            this.getBandeiras().subscribe(() => {
                (Boolean(sessionStorage.getItem('credenciamentoEdicao'))) ? this.edicao = true : this.analise = true;
                const condicoes = JSON.parse(sessionStorage.getItem('condicaoComercial'));
                this.preencheTaxasExistentes(condicoes);
            });
        } else {
            zip(this.getBandeiras(), this.getTaxasNovoCredenciamento()).subscribe(([bandeiras, taxas]) => {
    ...
    preencheTaxasExistentes(condicoes) {
        ...
        this.taxas.administrativas.sort((t1, t2) => t1.opcoesParcelamento.minimoParcelas - t2.opcoesParcelamento.minimoParcelas);
    ...
    criaControlsTabela() {
        this.bandeiras.forEach(b => {
            ...
            this.taxas.administrativas.filter(d => d.bandeira.id === b.id).forEach(t => {
                this.taxasPorBandeira[b.id].push(t);

                this.tabela.addControl(`b${b.id}tx${t.id}`, new FormControl(t.valor - t.coeficiente, Validators.required));
            });
        });
    }
