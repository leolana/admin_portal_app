...
export class CredenciamentoDadosCadastraisComponent implements OnInit {
  ...
  customValidators = {
    faturamentoAnual: () => {
      ...
      const faturamentoValido = faturamentosDisponiveis.some(faturamento => {
    ...
    ticketMedio: () => {
      ...
      const ticketValido = ticketsDisponiveis.some(ticket => {
  ...
  checkIfEditing() {
    this.route.params.subscribe(params => {
      ...
      if (
        this.temPassoAFrente &&
        !params.back &&
        !params.edicao &&
        !params.analise &&
        currentUrl !== this.route.url.toString()
      ) {
  ...
  getFaturamentos() {
    this.service.getFaturamentos().subscribe(faturamentosDisponiveis => {
      this.opcoesFaturamentoCartao = faturamentosDisponiveis;
      this.faturamentoAnual.setValidators([
        Validators.required,
        this.customValidators.faturamentoAnual,
      ]);
  ...
  getTickets() {
    this.service.getTickets().subscribe(ticketsDisponiveis => {
  ...
  validForm(): boolean {
    Object.keys(this.formDadosCadastrais.controls).forEach(k =>
      this.formDadosCadastrais.controls[k].markAsDirty(),
    );
    Object.keys(this.formInformacoesFinanceiras.controls).forEach(k =>
      this.formInformacoesFinanceiras.controls[k].markAsDirty(),
    );
    ...
    if (this.formCredenciamento.invalid) {
      const controlsErrors = Object.keys(
        this.formCredenciamento.controls.dadosCadastrais['controls'],
      )
  ...
  atualizaDadosSessionStorage(): void {
    ...
    novoCadastro.informacoesFinanceiras = {
      faturamentoAnual: this.opcoesFaturamentoCartao.find(
        t => t.id === +this.formInformacoesFinanceiras.value.faturamentoAnual,
      ),
      ticketMedio: this.opcoesTicketMedio.find(
        t => t.id === +this.formInformacoesFinanceiras.value.ticketMedio,
      ),
    };
    ...
    session.aberturaNascimento =
      this.tipoPessoa === TiposPessoa.fisica ? this.dataNascimento.value : this.dataAbertura.value;
