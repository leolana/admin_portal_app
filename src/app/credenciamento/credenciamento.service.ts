...
export class CredenciamentoService {
  ...
  export(parametros: any): void {
    this.http
      .get(`${environment.apiUrl}/credenciamento/export`, {
        params: parametros,
        responseType: 'blob',
        observe: 'response',
      })
  ...
  download(resource, documento, tipo, indice, file) {
    this.http
      .get(`${environment.apiUrl}/${resource}/${documento}/arquivos/${tipo}/${indice}`, {
        responseType: 'blob',
      })
      .pipe(
        catchError(err => {
  ...
  obterTaxas(): Observable<any[]> {
    return this.http.get<any>(`${environment.apiUrl}/dominio/taxas`, {
      params: {
        ...
        faturamentoCartao: `${
          JSON.parse(sessionStorage.getItem('faturamento')).faturamentoAnual.id
        }`,
      },
    });
  }
  ...
  iniciarCredenciamento() {
    ...
    const passo =
      this.credenciamento.tipoPessoa === TiposPessoa.fisica
  ...
  enviarCredenciamento(condicaoComercial: ICondicaoComercial) {
    ...
    this.credenciamento.documentos.forEach(documento => {
      if (typeof documento.arquivo === 'string') {
        ...
        formData.append(documento.id, documento.arquivo, (documento as any).nomeArquivo);
      }
    });
  ...
  getSessionStorage() {
    ...
    this.credenciamento.dadosCadastrais.informacoesFinanceiras = JSON.parse(
      sessionStorage.getItem('faturamento'),
    );
    this.credenciamento.dadosCadastrais.contato = JSON.parse(sessionStorage.getItem('contato'));
    this.credenciamento.dadosCadastrais.socios = JSON.parse(
      sessionStorage.getItem('dadosSocietarios'),
    );
    this.credenciamento.instalacao = JSON.parse(sessionStorage.getItem('dadosInstalacao'));
    this.credenciamento.dadosCadastrais.domiciliosBancarios = JSON.parse(
      sessionStorage.getItem('domicilioBancario'),
    );
  ...
  converteArquivos() {
    ...
    if (docs) {
      docs.forEach(doc => {
        doc.arquivo = this.base64ToFile(doc.arquivo);
      });
  ...
  editarCredenciamento(id: number, status: number) {
    this.obterResumo(id).subscribe(data => {
      ...
      const domiciliosBancarios = data.domiciliosBancarios
        .map(d => ({
          id: d.id,
          bandeira: d.bandeira,
          banco: {
