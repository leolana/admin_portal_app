...
export class CredenciamentoPesquisaComponent implements OnInit {
  ...
  filter() {
    ...
    this.service.pesquisar(parameters).subscribe(credenciamentos => {
      ...
      credenciamentos.forEach(credenciamento => {
        credenciamento.dataAprovado = credenciamento.dataAprovado
          ? new DatePipe().transform(credenciamento.dataAprovado)
          : '-';
        credenciamento.statusCode = credenciamento.status;
        credenciamento.statusDescricao =
          CredenciamentoStatus.descricoes[credenciamento.status] || 'Pré-Cadastro';
  ...
  naoPodeEditar(item) {
    return (
      !item.statusCode ||
      item.statusCode === CredenciamentoStatus.pendente ||
      item.statusCode === CredenciamentoStatus.reprovado
    );
  }
}
