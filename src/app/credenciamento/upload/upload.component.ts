...
export class CredenciamentoUploadComponent implements OnInit {
  ...
  // COMPUTED
  get pessoaFisica() {
    return this.tipoPessoa.value === TiposPessoa.fisica;
  }
  get fichaCadastroFileName() {
    return this.getFileName('fichaCadastro');
  }
  get identidadeFileName() {
    return this.getFileName('identidade');
  }
  get comprovanteDeResidenciaFileName() {
    return this.getFileName('comprovanteDeResidencia');
  }
  get contratoSocialFileName() {
    return this.getFileName('contratoSocial');
  }
  get extratosBancariosFileName() {
    return this.getFileName('extratosBancarios');
  }
  ...
  submitForm() {
    Object.keys(this.dadosCredenciado.controls).forEach(k =>
      this.dadosCredenciado.controls[k].markAsDirty(),
    );
    Object.keys(this.arquivos.controls).forEach(k => this.arquivos.controls[k].markAsDirty());

    if (!this.formCredenciamento.valid) {
      return;
    }
    ...
    this.credenciamentoService.enviarProposta(formData).subscribe(data => {
      this.promptService.inform('Credenciamento enviado com sucesso').then(() => {
