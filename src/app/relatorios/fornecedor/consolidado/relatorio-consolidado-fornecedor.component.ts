...
export class RelatorioConsolidadoFornecedorComponent implements OnInit {
  ...
  lists = {
    periods: [{ id: 7, text: '7 dias' }, { id: 15, text: '15 dias' }, { id: 30, text: '30 dias' }],
  };
  ...
  antecipacoesDatatable = new Datatable({
    table: [
      ...
      {
        property: 'valorDescontoAntecipacao',
        description: 'Valor do Desconto de Antecipação',
        pipe: 'currency',
      },
  ...
  list() {
    ...
    this.relatorioService.getRelatorioConsolidadoFornecedor(where).subscribe(result => {
      this.result = result;
