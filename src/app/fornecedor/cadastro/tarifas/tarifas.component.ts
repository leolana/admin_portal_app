...
export class TarifasFornecedorComponent implements OnInit {
    ...
    ngOnInit() {
        this.route.params.subscribe(params => {
            ...
            if (this.fornecedor.taxas) {
                this.taxasEdicaoAntecipacao = this.fornecedor.taxas.antecipacao || this.fornecedor.taxas.
                    filter(t => t.participanteTaxaTipo === participanteTaxaTipo.antecipacao)[0];
                this.taxasEdicaoCessao = this.fornecedor.taxas.cessao || this.fornecedor.taxas.
                    filter(t => t.participanteTaxaTipo === participanteTaxaTipo.cessao);

                this.formTarifasAntecipacao.controls.taxa.setValue(this.taxasEdicaoAntecipacao.taxa);
                this.taxasEdicaoCessao.length ? this.taxasEdicaoCessao.forEach(taxa => this.adicionaTaxaCessao(taxa))
    ...
    adicionaTaxaCessao(taxa = null) {
        ...
        if (this.valorInicioCessaoMaior()) {
            this.notification.showErrorMessage('Valor de Cessão Inicial não pode ser superior que o Valor de Cessão Final');
            return;
        }
        if (this.taxasCessao.length) {
            if (this.taxasCessao[this.taxasCessao.length - 1].get('valorFim').value >= this.valorFinalMaximo) {
        ...
        const formTarifasCessao = new FormGroup({
            participanteTaxaId: new FormControl(taxa ? { value: taxa.id || taxa.participanteTaxaId, disabled: true } : null),
            usuarioCriacao: new FormControl(taxa ? { value: taxa.usuarioCriacao, disabled: true } : null),
            dataInclusao: new FormControl(taxa ? { value: taxa.createdAt || taxa.participanteTaxaId, disabled: true } : null),
            valorInicio: new FormControl(taxa ? { value: taxa.valorInicio, disabled: true } : lastValorFim, Validators.required),
            valorFim: new FormControl(taxa ? { value: taxa.valorFim, disabled: true } : '', Validators.required),
            taxa: new FormControl(taxa ? { value: taxa.taxa, disabled: true } : '', Validators.required),
            participanteTaxaTipo: new FormControl(taxa ? { value: taxa.participanteTaxaTipo, disabled: true } : ''),
        });
    ...
    excluirTaxaCessao(taxa, i) {
        if (taxa.disabled) {
            this.prompt.confirm('A taxa cadastrada será excluída. Deseja continuar?', 'Aviso')
                .then(yes => {
                    if (yes) {
    ...
    valoresCessaoInvalid() {
        for (let taxa = 0; taxa < this.taxasCessao.length; taxa++) {
            Object.keys(this.taxasCessao[taxa]['controls'])
                .forEach(k => this.taxasCessao[taxa]['controls'][k].markAsDirty());
    ...
    valorInicioCessaoMaior() {
        return this.taxasCessao.some(taxa =>
            +taxa.controls.valorInicio.value > +taxa.controls.valorFim.value
        );
    }
    ...
    mapFornecedor() {
        ...
        if (this.taxasEdicaoAntecipacao) {
            if (this.taxasEdicaoAntecipacao.taxa !== this.formTarifasAntecipacao.controls.taxa.value) {
                ...
                if (!this.taxasExcluidas.some(t => t.participanteTaxaTipo === participanteTaxaTipo.antecipacao
                    && t.taxa === this.taxasEdicaoAntecipacao.taxa)) {
                    this.taxasExcluidas.push(this.taxasEdicaoAntecipacao);
                }
            } else {
                this.formTarifasAntecipacao.addControl('id', new FormControl(this.taxasEdicaoAntecipacao.id));
            }
        }
