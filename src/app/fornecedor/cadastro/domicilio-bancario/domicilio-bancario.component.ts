...
export class DomicilioBancarioFornecedorComponent implements OnInit {
  ...
  // METHODS
  getBancos() {
    return this.dominioService.obterBancos().pipe(tap(bancos => (this.bancos = bancos)));
  }

  getBandeiras() {
    return this.dominioService.obterBandeiras().pipe(tap(bandeiras => (this.bandeiras = bandeiras)));
  }
  ...
  criaBandeirasForms() {
    this.bandeiras.forEach(bandeira => {
      ...
      if (this.fornecedor.domiciliosBancarios) {
        const domicilioBandeira = this.fornecedor.domiciliosBancarios.find(
          f => f.bandeiraId === bandeira.id,
        );
  ...
  confirmarDomicilio() {
    if (this.validaDomicilioForm()) {
      ...
      this.domicilio.value.forEach((group, i) => {
        ...
        const keep =
          actual.bancoId === (group.bancoId || group.banco) &&
        ...
        data.bandeiras = ((agrupado[chave] || {}).bandeiras || []).concat(
          group.bandeiraId || group.bandeira.id,
        );
      ...
      this.domicilioArquivos = Object.values(agrupado).map((domicilio: any) => {
        ...
        return {
          ...
          descricao: `${domicilio.bancoNome} - Ag. ${domicilio.agencia} / Conta ${
            domicilio.conta
          }-${domicilio.digito}`,
          control: control,
          values: domicilio,
  ...
  verificaArquivosExistentes() {
    ...
    if (documentosExistentes) {
      documentosExistentes
        .filter(doc => doc.id.indexOf('extratosBancarios') == 0)
        .forEach(doc => {
  ...
  alterarDomicilio() {
    ...
    if (this.extratosBancarios.controls.some(control => control.dirty)) {
      promise = this.prompt.confirm(
        'Os arquivos selecionados serão descartados. Deseja continuar?',
        'Aviso',
      );
    }
  ...
  bindFile(event, index) {
    ...
    const fileSaved = this.dominioService.obterFileName(
      file,
      'extratosBancarios',
      documento,
      'fornecedor',
  ...
  validateForm(): any {
    ...
    if (totalSize > MaxUploadSize) {
      this.notification.showAlertMessage(
        'Os arquivos selecionados ultrapassam o limite total de 20MB.',
      );
      return false;
    }
    return true;
