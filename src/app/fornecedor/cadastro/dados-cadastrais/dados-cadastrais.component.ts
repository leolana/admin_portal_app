...
export class DadosCadastraisFornecedorComponent implements OnInit, AfterViewInit {
    ...
    validateForm() {
        ...
        if (!this.formFornecedorCadastro.valid) {
            const errosControlesCadastrais = this._obterErrosFormulario(this.formFornecedorCadastro.controls.dadosCadastrais);
            const errosControlesContato = this._obterErrosFormulario(this.formFornecedorCadastro.controls.dadosContato);
