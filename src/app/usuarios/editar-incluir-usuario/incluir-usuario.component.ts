...
export class UsuarioEditarComponent implements OnInit {
    ...
    checkTypeEditUser(usuario: any): void {
        ...
        const roleBackoffice = usuario.roles
            .find(r => r == Roles.boAdministrador || r == Roles.boOperacoes);

        const roleEstabelecimento = usuario.roles
            .find(r => r == Roles.ecAdministrador || r == Roles.ecFinanceiro || r == Roles.ecCompras);

        const roleFornecedor = usuario.roles
            .find(r => r == Roles.fcAdministrador || r == Roles.fcFinanceiro || r == Roles.fcComercial);
    ...
    loggedUserIsBKO(): boolean {
        return this.authService.hasRole(this.roles.boAdministrador)
            && !this.authService.user.participanteEstabelecimento
            && !this.authService.user.participanteFornecedor;
    }

    loggedUserIsEc(): boolean {
        return this.authService.hasRole(this.roles.boAdministrador, this.roles.ecAdministrador)
            && this.authService.user.participanteEstabelecimento;
    }

    loggedUserIsFc(): boolean {
        return this.authService.hasRole(this.roles.boAdministrador, this.roles.fcAdministrador)
            && this.authService.user.participanteFornecedor;
    }
    ...
    async validate() {
        ...
        if (this.formUsuario.invalid) {
            ...
            this.notificationService.showAlertMessage('Verifique os campos obrigatórios antes de prosseguir!');
            return;
        }
        ...
        if (this.id) {
            ...
            if (result.status) {
                ...
                const messageAwareOfActivation = 'Ao editar um usuário Inativo, ele será Ativado. Deseja continuar?';
