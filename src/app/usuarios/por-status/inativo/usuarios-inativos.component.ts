...
export class UsuariosInativosComponent implements OnInit, OnDestroy {
  ...
  @Input() set filters(params: any) {
    if (!/inativos$/i.test(this.router.url)) {
      return;
    }

    this.filterParams = params;
    const datatable = this.usuariosInativos.internal;
    if (datatable.initialized) this.obter(params, datatable.pageSize, datatable.pageIndex);
  }
      ...
      if (userKeycloak) {
        ...
        Object.keys(this.dataValidation).forEach(user => {
          this.dataValidation[user] = Array.isArray(userKeycloak[user])
            ? userKeycloak[user].every(userItem => userAlpe[user].includes(userItem))
            : userKeycloak[user] === userAlpe[user];
        });
      } else {
        this.infoUserAlpe = userAlpe;
      }
      this.infoUserKeyclaok.infoErrada = Object.values(this.dataValidation).some(
  ...
  loggedUserIsBKO(): boolean {
    return (
      this.authService.hasRole(Roles.boAdministrador) &&
