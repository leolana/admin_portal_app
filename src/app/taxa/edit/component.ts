...
export class TaxaEditComponent implements OnInit {
  ...
  getPrazos(): Observable<Prazo[]> {
    return this.service.getTaxaPrazos().pipe(tap(arr => (this.prazos = arr)));
  }
  ...
  getTipoPessoas(): Observable<IdText[]> {
    ...
    return of(tipoPessoas).pipe(tap(arr => (this.tipoPessoas = arr)));
  }

  getOpcoesFaturamentoCartao(): Observable<IdText[]> {
    return this.dominio
      .obterOpcoesFaturamentoCartao()
      .pipe(tap(arr => (this.opcoesFaturamentoCartao = arr.reverse())));
  }

  getRanges() {
    return this.service.getTaxaRanges().pipe(tap(arr => (this.ranges = arr)));
  }
  ...
  amanha(): string {
    ...
    const tomorrow = DateTime.fromJSDate(today)
      .plus({ days: 1 })
      .toISO();
    return tomorrow;
  }
  ...
  createDynamicFormBandeiras(): void {
    ...
    this.bandeiras.forEach(bandeira => {
      ...
      this.opcoesFaturamentoCartao.forEach(faturamento => {
        const _faturamento = deep(_bandeira, 'faturamentos', {
          key: 'idFaturamento',
          id: faturamento.id,
        });
        form.addControl(
          'coeficienteFaturamento' + faturamento.id,
          new FormControl(_faturamento.coeficiente),
        );
      });
    ...
    this.tableTaxas = this.bandeiras.map(bandeira => ({
      ...
      prazos: this.prazos.map(prazo => {
        ...
        this.ranges.forEach(range => {
          ...
          formTaxas.addControl(
            `taxa${range.minimo}a${range.maximo}`,
            new FormControl(_value && _value.taxa),
          );
        });
  ...
  filtrar(): void {
    this.service.getByFilter(this.formFiltro.value).subscribe(item => {
      ...
      if (jaExiste) {
        this.prompt
          .confirm('Já existem Taxas cadastradas para essa Configuração. Deseja alterá-las?')
          .then(yes => {
            if (yes) {
  ...
  taxaAdministrativa(bandeira: IdText, prazoGroup: Prazo, range: TaxaRange): number {
    ...
    const coeficienteFaturamento =
      bandeiraForm.controls['coeficienteFaturamento' + this.opcaoFaturamentoSelecionada()];
    ...
    return (
      ...
      parseFloat(coeficienteFaturamento.value || 0) +
      parseFloat(prazo.value || 0)
    );
  }
  ...
  save() {
    ...
    this.bandeiras.forEach((bandeira, i) => {
      ...
      this.opcoesFaturamentoCartao.forEach(faturamento => {
        ...
        _faturamento.coeficiente =
          form.controls['coeficienteFaturamento' + faturamento.id].value || 0;
      });
    });
