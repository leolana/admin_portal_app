...
export class AuthService {
  ...
  getSessionToken(participanteId) {
    return this.http
      .post<any>(environment.apiUrl + '/initiate-session', { participanteId: participanteId })
      .pipe(
        flatMap(result => {
          this.setupTokens(result);
  ...
  setupTokens(tokens) {
    if (tokens.accessToken) localStorage.setItem(accessTokenKey, tokens.accessToken);

    if (tokens.refreshToken) localStorage.setItem(refreshTokenKey, tokens.refreshToken);

    if (tokens.sessionToken) localStorage.setItem(sessionTokenKey, tokens.sessionToken);
  }
