...
export class HeaderComponent implements OnInit, OnDestroy {
    ...
    refreshBulletNotification() {
        this.notificacoesService.getBullets()
            .subscribe(bullets => this.bulletsNotifications = bullets.count);
    }

    getSubTitle() {
        if (this.bulletsNotifications == 1)
            return `Você tem ${this.bulletsNotifications} notificação.`;

        if (this.bulletsNotifications > 1)
            return `Você tem ${this.bulletsNotifications} notificações.`;

        if (this.bulletsNotifications == 0)
            return `Você não possui novas notificações.`;
    }
    ...
    changeMembership() {
        this.authService
            .getSessionToken(this.membership.value)
            .subscribe(user => {
                this.user = user;
    ...
    impersonate() {
        this.dialogService
            .open(ImpersonateParticipanteComponent, {})
            .then(participanteId => {
                if (participanteId) {
...
export class ContentHeaderComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        if (this.subtitleEvent) {
            this.subtitleSubscription = this.subtitleEvent
                .subscribe(subtitle => {
