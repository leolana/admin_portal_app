...
export class CessaoService {
  ...
  obter(id: number): Observable<any> {
    const participanteId = this.auth.user.participante;
    const participante = this.auth.user.participanteEstabelecimento
      ? 'estabelecimento'
      : 'fornecedor';
    return this.http.get<any[]>(
      `${environment.apiUrl}/${participante}/${participanteId}/cessao/${id}`,
    );
  }
  ...
  alterar(cessao: ICessao): Observable<any> {
    const participanteId = this.auth.user.participante;
    const url = `${environment.apiUrl}/estabelecimento/${participanteId}/cessao/${
      cessao.id
    }/alterar`;
