...
export class SolicitarCessaoRecorrenteComponent implements OnInit {
  ...
  hasError = {
    ...
    dataExpiracao: () => {
      ...
      if (
        ...
        this.form.value.dataExpiracao >= this.form.value.dataVencimento
      ) {
      ...
      const atualDate = DateTime.fromJSDate(this.form.value.dataExpiracao).set({
        hour: 0,
        minute: 0,
        second: 0,
        millisecond: 0,
    ...
    valorCessao: () => {
      ...
      if (
        this.vinculo.valorMaximoExibicao &&
        this.form.value.valorCessao > this.vinculo.valorMaximoExibicao
      ) {
      ...
      if (
        this.form.value.valorMaximo &&
        this.form.value.valorCessao > this.form.value.valorMaximo
      ) {
  ...
  obterDisponibilidadeRecorrente() {
    this.service.checaPermissaoCessaoRecorrente(this.vinculo.id).subscribe(auth => {
      this.auth = auth;
  ...
  submit() {
    ...
    const solicitacao = () =>
      this.service.solicitarCessao(json).subscribe(cessao => {
