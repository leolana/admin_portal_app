...
export class SolicitarCessaoParceladoComponent implements OnInit {
  ...
  calcular(): void {
    ...
    this.service.buscarOpcoesDeParcelamento(json, this.vinculo.id).subscribe(opcoes => {
  ...
  submit(): void {
    ...
    this.service.solicitarCessaoParcelada(json, this.vinculo.id).subscribe(cessao => {
      if (cessao.id) {
